...
function drawFromData(sunwell, data, textureDir, outputDir, resolution, premium, overwrite) {
	for (let i in data) {
		var card = data[i];
		card.key = i;
		...
		if (!fs.existsSync(renderPath) || overwrite) {
...
function main() {
	...
	p.addArgument("--overwrite", {action: "storeTrue"});
